...
function InlineEdit({ value, placeholder, onChange, bright, large }) {
  ...
  if (editing) return (
    <input ref={ref} value={val}
      ...
      style={{ background:"#1a1a1a", border:"1px solid #444", color:"#fff",
               fontFamily:"'Barlow', sans-serif", fontSize: large ? "14px" : "13px",
               padding:"4px 8px", borderRadius:"4px", outline:"none",
               width:"100%", boxSizing:"border-box" }} />
  );
  return (
    <span onClick={() => setEditing(true)} title={value || placeholder}
      style={{ cursor:"text", color: value ? (bright||"#e8e8e8") : "#555",
               ...
               textOverflow: large ? "clip" : "ellipsis" }}>
      {value || placeholder}
    </span>
  );
}
...
function exportToCSV(verts) {
  ...
  verts.forEach(v => {
    ...
    v.subtasks.forEach(s => rows.push([
      '"'+v.title+'"', '"'+v.milestone+'"', '"'+s.name+'"',
      '"'+s.assignee+'"', '"'+s.due+'"', '"'+s.status+'"',
      s.blocked?"Yes":"No", pct+"%",
    ]));
  });
...
export default function App() {
  ...
  const updateSub = (vi,si,field,val) => setVerts(prev => prev.map((v,i) => i!==vi ? v : { ...v, subtasks:v.subtasks.map((s,j) => j!==si ? s : {...s,[field]:val}) }));
  const cycleStatus = (vi,si) => setVerts(prev => prev.map((v,i) => i!==vi ? v : { ...v, subtasks:v.subtasks.map((s,j) => { if(j!==si) return s; const next=STATUS_CYCLE[(STATUS_CYCLE.indexOf(s.status)+1)%STATUS_CYCLE.length]; return {...s,status:next}; }) }));
  const toggleBlocked = (vi,si) => setVerts(prev => prev.map((v,i) => i!==vi ? v : { ...v, subtasks:v.subtasks.map((s,j) => j!==si ? s : {...s,blocked:!s.blocked}) }));
  const addSub = (vi) => setVerts(prev => prev.map((v,i) => i!==vi ? v : { ...v, subtasks:[...v.subtasks,{id:++uid,name:"New Task",assignee:"",due:"",status:"To Do",blocked:false}] }));
  const removeSub = (vi,si) => setVerts(prev => prev.map((v,i) => i!==vi ? v : { ...v, subtasks:v.subtasks.filter((_,j) => j!==si) }));
  const updateTitle = (vi,val) => setVerts(prev => prev.map((v,i) => i!==vi ? v : {...v,title:val}));
  const updateMilestone = (vi,val) => setVerts(prev => prev.map((v,i) => i!==vi ? v : {...v,milestone:val}));
  const toggleCollapse = (vi) => setVerts(prev => prev.map((v,i) => i!==vi ? v : {...v,collapsed:!v.collapsed}));
  const addVertical = () => setVerts(prev => [...prev,{id:++uid,title:"New Vertical",milestone:"",collapsed:false,subtasks:[{id:++uid,name:"New Task",assignee:"",due:"",status:"To Do",blocked:false}]}]);
  const removeVertical = (vi) => { if(!window.confirm("Remove this vertical and all its tasks?")) return; setVerts(prev => prev.filter((_,i) => i!==vi)); };
  ...
  const workloadMap = {};
  verts.forEach((v,vi) => v.subtasks.forEach(s => {
    const who=(s.assignee||"").trim(); if(!who) return;
  ...
  const CSS = [
    ...
    ".task-row { border-bottom:1px solid #161616; transition:background 0.12s; }",
    ".task-row:hover { background:rgba(255,255,255,0.05) !important; }",
    ".task-row:hover .del-btn { opacity:1 !important; }",
    ".drag-handle { cursor:grab; color:#444; font-size:13px; padding:2px 4px; user-select:none; flex-shrink:0; transition:color 0.15s; }",
    ".drag-handle:hover { color:#888; }",
    ".vert-card { background:#0d0d0d; border-radius:8px; overflow:hidden; display:flex; flex-direction:column; }",
    ".filter-btn { background:transparent; border:1px solid #252525; color:#555; padding:5px 12px; border-radius:4px; cursor:pointer; font-family:'Barlow Condensed'; font-size:11px; font-weight:700; letter-spacing:0.08em; transition:all 0.15s; }",
    ...
    ".export-btn { background:transparent; border:1px solid #2a2a2a; color:#666; padding:6px 14px; border-radius:4px; cursor:pointer; font-family:'Barlow Condensed'; font-size:11px; font-weight:700; letter-spacing:0.1em; transition:all 0.15s; }",
    ".export-btn:hover { border-color:#3de8ff; color:#3de8ff; }",
    ".task-name-input { background:#1a1a1a; border:1px solid #444; color:#fff; font-family:'Barlow', sans-serif; font-size:14px; padding:4px 8px; border-radius:4px; outline:none; width:100%; box-sizing:border-box; }",
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#000", padding:"28px 22px", fontFamily:"'Barlow', sans-serif" }}>
      ...
      {/* HEADER */}
      <div className="header-row" style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"24px", flexWrap:"wrap", gap:"12px" }}>
        <div>
        ...
        <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:"10px" }}>
          <div style={{ textAlign:"right" }}>
            ...
            <div style={{ fontFamily:"'Barlow'", fontSize:"12px", color:"#777", marginTop:"2px" }}>{doneTasks} of {totalTasks} tasks complete</div>
          </div>
      ...
      <div style={{ display:"flex", gap:"6px", marginBottom:"20px", flexWrap:"wrap", alignItems:"center" }}>
        <span style={{ fontFamily:"'Barlow Condensed'", fontSize:"10px", color:"#444", letterSpacing:"0.12em", marginRight:"4px" }}>FILTER:</span>
      ...
      {/* VERTICALS GRID */}
      <div className="vert-grid" style={{ display:"grid", gridTemplateColumns:"repeat("+Math.min(verts.length,3)+",1fr)", gap:"16px", marginBottom:"24px" }}>
        {verts.map((v,vi) => {
          const pct   = getProgress(v.subtasks);
          ...
          return (
            <div key={v.id} className="vert-card" style={{ border:"1px solid "+color+"33", borderLeft:"3px solid "+color }}>

              {/* COLUMN HEADER */}
              <div style={{ background:color+"0e", padding:"14px 16px", borderBottom:"1px solid "+color+"22" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"10px" }}>
                  <InlineEdit value={v.title} placeholder="Vertical title" onChange={val=>updateTitle(vi,val)} bright={color} large />
                  <div style={{ display:"flex", gap:"6px", alignItems:"center", flexShrink:0, marginLeft:"10px", marginTop:"2px" }}>
                    <span style={{ fontFamily:"'Barlow Condensed'", fontSize:"10px", fontWeight:700, color:r.color, letterSpacing:"0.1em", background:r.color+"18", border:"1px solid "+r.color, borderRadius:"3px", padding:"2px 7px" }}>{r.label}</span>
                    <button onClick={()=>toggleCollapse(vi)} title={v.collapsed?"Expand":"Collapse"} style={{ background:"none", border:"none", color:"#555", cursor:"pointer", fontSize:"13px", padding:"2px 3px", lineHeight:1 }}>{v.collapsed?"▶":"▼"}</button>
                    <button onClick={()=>removeVertical(vi)} title="Remove vertical" style={{ background:"none", border:"none", color:"#2a2a2a", cursor:"pointer", fontSize:"17px", padding:"2px 3px", lineHeight:1, transition:"color 0.15s" }} onMouseEnter={e=>e.target.style.color="#ff5f5f"} onMouseLeave={e=>e.target.style.color="#2a2a2a"}>×</button>
                  </div>
                </div>
                <Bar pct={pct} color={color} />
                <div style={{ display:"flex", justifyContent:"space-between", marginTop:"7px" }}>
                  <span style={{ fontFamily:"'Barlow'", fontSize:"12px", color:"#ddd" }}><span style={{ fontWeight:600, color:"#fff" }}>{pct}%</span> complete</span>
                  <span style={{ fontFamily:"'Barlow'", fontSize:"12px", color:"#555" }}>{done}/{v.subtasks.length} done</span>
                </div>
              </div>

              {/* COLLAPSED */}
              {v.collapsed && (
                <div style={{ padding:"12px 16px", color:"#444", fontFamily:"'Barlow'", fontSize:"12px", fontStyle:"italic", textAlign:"center" }}>
                  {v.subtasks.length} tasks hidden · click ▶ to expand
                </div>
              )}
              ...
              {!v.collapsed && (
                <div style={{ flex:1 }}>
                  {/* MILESTONE */}
                  <div style={{ display:"flex", alignItems:"center", gap:"8px", padding:"8px 16px", borderBottom:"1px solid "+color+"1a", background:"rgba(0,0,0,0.25)" }}>
                    <span style={{ fontFamily:"'Barlow Condensed'", fontSize:"10px", fontWeight:700, color:lc, letterSpacing:"0.12em", flexShrink:0 }}>MILESTONE</span>
                    <input type="date" value={v.milestone} onChange={e=>updateMilestone(vi,e.target.value)}
                      style={{ background:"transparent", border:"none", borderBottom:"1px solid "+color+"2a", color:v.milestone?"#fff":"#444", cursor:"pointer", fontFamily:"'Barlow'", fontSize:"12px", outline:"none", flex:1, colorScheme:"dark" }} />
                    {v.milestone && (
                      <span style={{ fontFamily:"'Barlow'", fontSize:"11px", color:lc, flexShrink:0 }}>TARGET: {new Date(v.milestone+"T12:00:00").toLocaleDateString("en-US",{month:"short",day:"numeric",year:"numeric"})}</span>
                    )}
                  </div>

                  {/* EMPTY FILTER STATE */}
                  {filteredSubs.length===0 && (
                    <div style={{ padding:"16px", color:"#444", fontFamily:"'Barlow'", fontSize:"12px", fontStyle:"italic", textAlign:"center" }}>No tasks match this filter.</div>
                  )}

                  {/* TASK ROWS - card layout: task name full width, metadata below */}
                  {filteredSubs.map(s => {
                    ...
                    return (
                      <div key={s.id} className="task-row"
                        draggable onDragStart={()=>onDragStart(vi,si)} onDragOver={e=>onDragOver(e,vi,si)} onDragEnd={onDragEnd}
                        style={{ padding:"10px 14px 10px 12px",
                                 background: s.blocked?"rgba(255,95,95,0.05)":isOverdue?"rgba(255,107,53,0.05)":"transparent",
                                 opacity:isDragging?0.35:1, transition:"opacity 0.15s, background 0.12s",
                                 borderLeft:"2px solid "+(s.blocked?"#ff5f5f33":isOverdue?"#ff6b3533":"transparent") }}>

                        {/* ROW TOP: drag handle + task name + delete */}
                        <div style={{ display:"flex", alignItems:"flex-start", gap:"6px", marginBottom:"8px" }}>
                          <span className="drag-handle" title="Drag to reorder">⠇</span>
                          <div style={{ flex:1, minWidth:0 }}>
                            <InlineEdit value={s.name} placeholder="Task name" onChange={val=>updateSub(vi,si,"name",val)} large />
                          </div>
                          <button className="del-btn" onClick={()=>removeSub(vi,si)}
                            style={{ opacity:0, background:"none", color:"#ff5f5f", cursor:"pointer",
                                     fontSize:"16px", lineHeight:1, padding:"0 2px", flexShrink:0,
                                     border:"none", transition:"opacity 0.15s", marginTop:"1px" }}>×</button>
                        </div>

                        {/* ROW BOTTOM: metadata chips */}
                        <div style={{ display:"flex", alignItems:"center", gap:"8px", flexWrap:"wrap", paddingLeft:"22px" }}>
                          ...
                          <div style={{ display:"flex", alignItems:"center", gap:"4px", minWidth:"60px" }}>
                            <span style={{ fontFamily:"'Barlow Condensed'", fontSize:"9px", color:"#444", letterSpacing:"0.1em", flexShrink:0 }}>WHO</span>
                            <InlineEdit value={s.assignee} placeholder="—" onChange={val=>updateSub(vi,si,"assignee",val)} bright={lc} />
                          </div>
                          {/* DIVIDER */}
                          <span style={{ width:"1px", height:"12px", background:"#252525", flexShrink:0 }} />
                          {/* DUE DATE */}
                          <div style={{ display:"flex", alignItems:"center", gap:"4px" }}>
                            <span style={{ fontFamily:"'Barlow Condensed'", fontSize:"9px", color:"#444", letterSpacing:"0.1em", flexShrink:0 }}>DUE</span>
                            <input type="date" value={s.due} onChange={e=>updateSub(vi,si,"due",e.target.value)}
                              style={{ background:"transparent", border:"none",
                                       borderBottom:"1px solid "+(isOverdue?"#ff5f5f":"#2a2a2a"),
                                       color:isOverdue?"#ff6b35":s.due?"#ccc":"#444",
                                       fontFamily:"'Barlow'", fontSize:"12px", outline:"none",
                                       cursor:"pointer", colorScheme:"dark", width:"110px" }} />
                          </div>
                          {/* DIVIDER */}
                          <span style={{ width:"1px", height:"12px", background:"#252525", flexShrink:0 }} />
                          {/* STATUS */}
                          ...
                          <button onClick={()=>toggleBlocked(vi,si)} title={s.blocked?"Unblock":"Flag blocked"}
                            style={{ opacity:s.blocked?1:0.18, background:"none", border:"none",
                                     cursor:"pointer", fontSize:"14px", padding:0, color:"#ff5f5f",
                                     transition:"opacity 0.15s", marginLeft:"2px" }}>⊘</button>
                        </div>
                      </div>
                    );
                  })}
                  ...
                  {/* ADD SUBTASK */}
                  <button onClick={()=>addSub(vi)}
                    style={{ background:"transparent", border:"none", color:lc, cursor:"pointer",
                             fontFamily:"'Barlow Condensed'", fontSize:"11px", fontWeight:700,
                             letterSpacing:"0.1em", padding:"11px 16px", display:"block",
                             width:"100%", textAlign:"left", borderTop:"1px solid #161616",
      ...
      <div style={{ marginTop:"8px", marginBottom:"16px" }}>
        <button onClick={()=>setShowWorkload(!showWorkload)}
          style={{ background:"transparent", border:"1px solid #1e1e1e", color:"#555", padding:"7px 14px",
                   borderRadius:"3px", cursor:"pointer", fontFamily:"'Barlow Condensed'", fontSize:"11px",
                   fontWeight:700, letterSpacing:"0.1em", transition:"all 0.15s" }}
          onMouseEnter={e=>e.currentTarget.style.borderColor="#333"}
          onMouseLeave={e=>e.currentTarget.style.borderColor="#1e1e1e"}>
          {showWorkload?"▲":"▼"} SHOW OWNER WORKLOAD
        </button>
        {showWorkload && (
          <div style={{ marginTop:"14px", paddingTop:"14px", borderTop:"1px solid #141414" }}>
            {workloadEntries.length===0
              ? <div style={{ fontFamily:"'Barlow'", fontSize:"13px", color:"#777" }}>No assignees set. Add names to the WHO field on any task above.</div>
              : workloadEntries.map(([who,tasks]) => (
                <div key={who} style={{ marginBottom:"16px" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:"10px", marginBottom:"8px" }}>
                    ...
                    <span style={{ fontFamily:"'Barlow'", fontSize:"12px", color:"#666" }}>{tasks.length} task{tasks.length!==1?"s":""}</span>
                  ...
                  {tasks.map((t,ti) => (
                    <div key={ti} style={{ display:"flex", alignItems:"center", gap:"10px", padding:"6px 0", borderBottom:"1px solid #0f0f0f" }}>
                      ...
                      <span style={{ fontFamily:"'Barlow'", fontSize:"13px", color:"#e0e0e0", flex:1 }}>{t.name}</span>
                      <span style={{ fontFamily:"'Barlow Condensed'", fontSize:"11px", color:"#555" }}>{t.vertical}</span>
      ...
      <div style={{ display:"flex", gap:"20px", paddingTop:"14px", borderTop:"1px solid #141414", alignItems:"center", flexWrap:"wrap" }}>
        {STATUS_CYCLE.map(s => (
          <div key={s} style={{ display:"flex", alignItems:"center", gap:"6px" }}>
            ...
            <span style={{ fontFamily:"'Barlow Condensed'", fontSize:"11px", fontWeight:700, color:"#666", letterSpacing:"0.08em" }}>{s.toUpperCase()}</span>
          </div>
        ))}
        ...
        <span style={{ fontFamily:"'Barlow Condensed'", fontSize:"10px", color:"#2a2a2a", letterSpacing:"0.08em", marginLeft:"auto" }}>CLICK STATUS TO CYCLE · CLICK ANY FIELD TO EDIT · DRAG ⠇ TO REORDER · HOVER TO DELETE</span>
      </div>
    </div>
  );
}
