...
function loadState() {
  try {
    ...
    return saved ? JSON.parse(saved) : DEFAULT_VERTICALS;
  } catch { return DEFAULT_VERTICALS; }
}
...
function ColLabel({ children, color }) {
  return (
    <span style={{ fontFamily:"'Barlow Condensed'", fontSize:"10px", fontWeight:700, color, letterSpacing:"0.16em" }}>
      {children}
    </span>
  );
}
        ...
        {verts.map((v, vi) => {
          ...
          return (
            <div key={v.id} style={{ background:"#0a0a0a", border:`1px solid ${color}44`, borderLeft:`3px solid ${color}`, borderRadius:"4px", padding:"14px 16px" }}>
              ...
              {v.milestone && (
                <div style={{ marginTop:"6px", fontFamily:"'Barlow Condensed'", fontSize:"12px", fontWeight:600, color, letterSpacing:"0.06em" }}>
